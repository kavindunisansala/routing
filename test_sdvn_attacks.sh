...
# Tests attacks by compromised data plane nodes (vehicles/RSUs) in SDVN architecture# SDVN Attack Testing Script

# Controllers remain trusted - only data plane nodes are malicious# Tests all 5 SDVN attack types and their mitigation solutions
...
print_header() {# Configuration
    ...
    echo ""NS3_PATH="${NS3_PATH:-.}"  # Default to current directory (NS-3 root)

    echo "================================================================"ROUTING_SCRIPT="routing"

    echo "$1"RESULTS_DIR="./results_$(date +%Y%m%d_%H%M%S)"

    echo "================================================================"SIM_TIME=60  # Simulation time in seconds
...
print_info() {
    ...
    echo -e "${BLUE}ℹ${NC} $1"# Performance metrics thresholds

}BASELINE_PDR_MIN=0.85          # Minimum baseline PDR: 85%

ATTACK_PDR_MAX=0.60            # Maximum PDR under attack: 60%

print_success() {MITIGATION_PDR_MIN=0.75        # Minimum PDR with mitigation: 75%

    echo -e "${GREEN}✓${NC} $1"DETECTION_ACCURACY_MIN=0.80    # Minimum detection accuracy: 80%
...
print_warning() {
    ...
    echo -e "${YELLOW}⚠${NC} $1"print_header() {

}    echo -e "\n${BLUE}================================================================${NC}"

    echo -e "${BLUE}$1${NC}"

print_header "SDVN DATA PLANE ATTACK TESTING SUITE"    echo -e "${BLUE}================================================================${NC}\n"
...
# Configuration    echo -e "${GREEN}✓ $1${NC}"

TIMESTAMP=$(date +%Y%m%d_%H%M%S)}

RESULTS_DIR="./sdvn_results_${TIMESTAMP}"

ROUTING_SCRIPT="routing"print_error() {

SIM_TIME=100    echo -e "${RED}✗ $1${NC}"

VEHICLES=18}

RSUS=10

ARCHITECTURE=0  # 0=centralized SDVNprint_warning() {
...
test_baseline() {# Create results directory structure

    local output_dir="$RESULTS_DIR/baseline"setup_results_dir() {

    mkdir -p "$output_dir/logs"    print_info "Creating results directory: $RESULTS_DIR"
